...
class ArbolBinarioBusqueda:
    ...
    def PorNiveles(self):
        ...
        valores = []
        pendientes = deque([self.__raiz] if self.__raiz else [])
        while pendientes:
            nodo = pendientes.popleft()
            valores.append(nodo.get_dato())
            for hijo in (nodo.get_izquierdo(), nodo.get_derecho()):
                if hijo is not None:
    ...
    def CantidadHojas(self):
        ...
        def contar(nodo):
            ...
            if self.EsHoja(nodo):
                return 1
            return contar(nodo.get_izquierdo()) + contar(nodo.get_derecho())
        return contar(self.__raiz)

    def EsBalanceado(self):
        """Consulta el balance; este ABB no realiza rotaciones."""
        def altura(nodo):
            if nodo is None:
                return -1
            izquierda = altura(nodo.get_izquierdo())
            derecha = altura(nodo.get_derecho())
            if izquierda == -2 or derecha == -2:
                return -2
            if abs(izquierda - derecha) > 1:
                return -2
            return 1 + max(izquierda, derecha)
        return altura(self.__raiz) != -2

    def Camino(self, valor):
        """Devuelve los nodos examinados al buscar, exista o no el valor."""
        camino = []
        nodo = self.__raiz
        while nodo is not None:
            ...
            if valor == dato:
                break
            nodo = nodo.get_izquierdo() if valor < dato else nodo.get_derecho()
        return camino
